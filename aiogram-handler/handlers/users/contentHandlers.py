...
download_path=Path().joinpath("download","categories")
...
async def any_handler(message:Message):
    await message.reply(f"{message.content_type} Kantakt qabul qilindi")

@dp.message_handler(content_types=ContentType.AUDIO)
async def any_handler(message:Message):
    await message.reply(f"{message.content_type} ovoz qabul qilindi")
